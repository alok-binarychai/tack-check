...
class SettingController extends Controller
{
    public function index(Request $request){
        try {
            ...
            if ($request->has('section_zero_image')) {
                $file = $request->file('section_zero_image');
                $file->storeAs('public/images', $file->getClientOriginalName());
                Setting::where(['key' => 'section_zero_image', 'page' => $request->page])->update(['key' => 'section_zero_image', 'value' => $file->getClientOriginalName()]);
            }
            ...
            if ($request->has('section_after_school_file')) {
                $file = $request->file('section_after_school_file');
                $file->storeAs('public/files', $file->getClientOriginalName());
                Setting::where(['key' => 'section_after_school_file', 'page' => $request->page])->update(['key' => 'section_after_school_file', 'value' => $file->getClientOriginalName()]);
            }
